        ...
        if (!worldIn.isRemote) {
            ...
            if (tileEntity != null) {
                if (tileEntity instanceof TileEntityOwnedBlock) {
                    ...
                    playerIn.addChatMessage(new TextComponentString("Owner name is \"" + te.getOwnerName() + "\" and UUID is " + te.getOwner()));
                }
                if (tileEntity instanceof TileEntityElectric) {
                    TileEntityElectric te = (TileEntityElectric) tileEntity;
                    playerIn.addChatMessage(new TextComponentString("Stored Energy: " + te.getEnergyLevel(EnumFacing.DOWN) + "/" + te.getMaxEnergyLevel(EnumFacing.DOWN)));
                    playerIn.addChatMessage(new TextComponentString("Stored EU: " + te.getStorageEU()));
                }
                ...
                if (tileEntity instanceof IDebugTile) {
                    ...
                    if (debugInfo.size() >= 1) {
                        for (String debug : debugInfo) {
                            playerIn.addChatMessage(new TextComponentString(debug));
                        }
                    }
                }
            }
